...
use Gigablah\Silex\OAuth\Security\Authentication\OsvAuthenticationSuccessHandler;
use Gigablah\Silex\OAuth\Security\Authentication\OsvAuthenticationFailureHandler;
...
class OAuthServiceProvider implements ServiceProviderInterface
{
    public function register(Application $app)
    {
        ...
        $app['oauth.client_auth_route'] = '_auth_service_client_auth';
        ...
        $app['oauth.user_provider_listener'] = $app->share(function ($app) {
            $userProvider = new UserProviderListener();
            $userProvider->setApp($app);
            $userProvider->setExternalUser($app['oauth.externalUser']);
            return $userProvider;
        });
        ...
        $app['security.authentication_listener.oauth._proto'] = $app->protect(function ($name, $options) use ($app) {
            return $app->share(function () use ($app, $name, $options) {
                ...
                $options['client_auth_route'] = $app['oauth.client_auth_route'];
                ...
                if ($app['oauth.register_routes']) {
                    ...
                    $app->post(
                        isset($options['client_auth_path']) ? $options['client_auth_path'] : '/auth/{service}/client_auth',
                        function () {}
                    )->bind($options['client_auth_route']);
                }
                $app['security.authentication.success_handler._proto'] = $app->protect(function ($name, $options) use ($app) {
                    return $app->share(function () use ($name, $options, $app) {
                        $handler = new OsvAuthenticationSuccessHandler(
                            $app,
                ...
                $app['security.authentication.failure_handler._proto'] = $app->protect(function ($name, $options) use ($app) {
                    return $app->share(function () use ($name, $options, $app) {
                        return new OsvAuthenticationFailureHandler(
                            $app,
                            $app['security.http_utils'],
                            $options,
                            $app['logger']
