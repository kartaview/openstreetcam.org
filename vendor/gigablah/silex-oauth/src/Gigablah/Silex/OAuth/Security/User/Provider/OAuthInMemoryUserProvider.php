...
class OAuthInMemoryUserProvider implements UserProviderInterface, OAuthUserProviderInterface
{
    ...
    public function __construct(array $users = array(), array $credentials = array())
    {
        foreach ($users as $username => $attributes) {
            ...
            $user = new StubUser($username,1 , $password, $email, $roles, $enabled, true, true, true);
    ...
    public function loadUserByUsername($username)
    {
        if (isset($this->users[strtolower($username)])) {
            ...
            $user = new StubUser($username, 1, '', $username . '@example.org', array('ROLE_USER'), true, true, true, true);
            $this->createUser($user);
        }
        return new StubUser($user->getUsername(),1, $user->getPassword(), $user->getEmail(), $user->getRoles(), $user->isEnabled(), $user->isAccountNonExpired(), $user->isCredentialsNonExpired(), $user->isAccountNonLocked());
    }

    /**
     * {@inheritdoc}
     */
    public function loadUserByOAuthCredentials(OAuthTokenInterface $token, $externalUser = null, $app = null)
    {
        ...
        if ($externalUser) {
            $externalUser->getByUsername($app, $token->getUsername());
        }

        $roles = $externalUser->getRole()?(array)$externalUser->getRole():array('ROLE_USER');
        $userId = $externalUser->getId()?$externalUser->getId():0;
        $user = new StubUser($token->getUsername(),$token->getUid(), '', $token->getService(), $roles, true, true, true, true, $externalUser->getId());
        $this->createUser($user);
