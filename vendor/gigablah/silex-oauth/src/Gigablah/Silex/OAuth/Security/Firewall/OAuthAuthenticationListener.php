    ...
    protected function attemptAuthentication(Request $request)
    {
        ...
        if ($this->httpUtils->checkRequestPath($request, $this->options['client_auth_route'])) {
            if (!$request->request->has('request_token')) {
                throw new AuthenticationException('Could not retrieve access request_token.');
            }
            if (!$request->request->has('secret_token')) {
                throw new AuthenticationException('Could not retrieve access secret_token.');
            }
            $requestToken = $request->request->get('request_token');
            $secretToken = $request->request->get('secret_token');
            $token = new StdOAuth1Token($requestToken);//'gbkzHMVR67Vun6Xh07b4BJAv9kLTi2UZ9TnMCVw9');
            $token->setRequestToken($requestToken);//'gbkzHMVR67Vun6Xh07b4BJAv9kLTi2UZ9TnMCVw9');
            $token->setRequestTokenSecret($secretToken);//'YmVmkfTJ7TMrhwP0WNbSVpNbHrAG450XxXPaibcH');
            $token->setAccessTokenSecret($secretToken);//'YmVmkfTJ7TMrhwP0WNbSVpNbHrAG450XxXPaibcH');
            $oauthService->getStorage()->storeAccessToken('OpenStreetMap', $token);
        }
